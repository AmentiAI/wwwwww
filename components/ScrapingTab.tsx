...
import { RefreshCw, Mail, Globe, Search, FileText, Download, Trash2, AlertTriangle, Database, Bug } from 'lucide-react'
...
export default function ScrapingTab() {
  ...
  const [emailFilter, setEmailFilter] = useState<'all' | 'with-emails' | 'no-emails'>('all')
  ...
  const fetchDomains = async (contacted?: boolean) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        applyEmailFilter(data.domains, emailFilter)
  ...
  const applyEmailFilter = (domains: Domain[], filter: 'all' | 'with-emails' | 'no-emails') => {
    let filtered = domains
    
    switch (filter) {
      case 'with-emails':
        filtered = domains.filter(d => d.emails.length > 0)
        break
      case 'no-emails':
        filtered = domains.filter(d => d.emails.length === 0)
        break
    ...
    console.log(`🔍 DEV DEBUG: Applied filter '${filter}': ${filtered.length}/${domains.length} domains shown`)
  }

  const handleEmailFilterChange = (filter: 'all' | 'with-emails' | 'no-emails') => {
    setEmailFilter(filter)
    applyEmailFilter(domains, filter)
  }
  ...
  const deleteDomain = async (domainId: string, domainUrl: string) => {
    if (!confirm(`🗑️ DEV DEBUG: Delete domain "${domainUrl}" and all related emails/analysis?`)) {
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`✅ Deleted ${domainUrl} (${data.deletedEmails} emails)`)
        console.log('🗑️ DEV DEBUG: Domain deleted:', data)
  ...
  const deleteAllDomains = async () => {
    if (!confirm('🚨 DEV DEBUG: DELETE ALL DOMAINS AND DATA? This cannot be undone!')) {
      return
    }
    
    if (!confirm('⚠️ FINAL WARNING: This will delete ALL domains, emails, and analyses. Are you absolutely sure?')) {
      return
    }

    try {
      const response = await fetch('/api/domains?all=true', {
        method: 'DELETE'
      })
      ...
      if (response.ok) {
        toast.success('🗑️ All data deleted successfully')
        console.log('🗑️ DEV DEBUG: All data deleted:', data)
        fetchDomains() // Refresh the list
      } else {
...
Generated by Domain Email Scraper
  ...
  // Initialize filtered domains when domains change
  useEffect(() => {
    applyEmailFilter(domains, emailFilter)
  }, [domains, emailFilter])
    ...
    <div className="space-y-6">
      ...
      <div className="bg-red-50 border-2 border-red-200 rounded-lg p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h1 className="text-lg font-bold text-red-800">🚨 DEVELOPER DEBUG CENTER 🚨</h1>
        </div>
        <p className="text-red-700 text-sm">
          This is a development tool with full data transparency. All operations are logged to console. 
          Not intended for public use - debug information is exposed.
        </p>
      </div>
      ...
      <div className="bg-red-100 border-2 border-red-300 rounded-lg p-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-md flex items-center gap-2 font-medium"
          >
        ...
        <p className="text-red-700 text-sm mt-2">
          Permanently delete all domains, emails, and analyses from the database.
        </p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => handleEmailFilterChange('with-emails')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
              emailFilter === 'with-emails'
                ? 'bg-green-100 text-green-800 border border-green-200'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            }`}
          >
            📧 With Emails ({domains.filter(d => d.emails.length > 0).length})
          </button>
          <button
            onClick={() => handleEmailFilterChange('no-emails')}
            className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${
              emailFilter === 'no-emails'
                ? 'bg-red-100 text-red-800 border border-red-200'
                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
            }`}
          >
            ❌ No Emails ({domains.filter(d => d.emails.length === 0).length})
          </button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredDomains.map((domain) => (
          <div key={domain.id} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded p-2 mb-3 text-xs">
              ...
              <div className="text-yellow-700 space-y-1">
                ...
                <div><strong>Created:</strong> {new Date(domain.createdAt).toISOString()}</div>
                <div><strong>Contacted:</strong> {domain.contacted ? 'Yes' : 'No'}</div>
                <div><strong>Email Count:</strong> {domain.emails.length}</div>
              </div>
            </div>
                  ...
                  {domain.emails.map((email) => (
                    <div
                      ...
                      className={`p-3 rounded border ${
                        email.contacted ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'
                      }`}
                    >
                      <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className="text-xs text-gray-500 space-y-1">
                        <div><strong>Email ID:</strong> {email.id}</div>
                        <div><strong>Contacted:</strong> {email.contacted ? 'Yes' : 'No'}</div>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {filteredDomains.length === 0 && !loading && (
        <div className="text-center py-8 text-gray-500">
          {domains.length === 0 
            ...
            : `No domains match the current filter (${emailFilter}). Try changing the filter above.`
          }
        </div>
      )}
    </div>
